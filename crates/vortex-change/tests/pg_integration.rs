...
async fn create_test_cr(env: &TestEnv, requester: Uuid) -> (Uuid, String, Uuid) {
    ...
    .bind(cr_id)
    .bind(&number)
    .bind("Replace transformer TX-01")
    .bind("Annual preventive maintenance, swap primary transformer.")
    .bind("maintenance")
    .bind("medium")
    .bind(Some("Restore from last-known-good snapshot and reroute load to TX-02."))
    .bind(requester)
    .bind(instance.id.0)
    .bind(SYSTEM_COMPANY_ID)
    .bind(now)
