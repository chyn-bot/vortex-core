# Reconciliation (vortex-recon)

Scaffolded by `vortex scaffold plugin recon`. Already registered
...
DATABASE_URL=<tenant-url> ./target/debug/vortex module install recon   # install is an operator decision
```

(Or install from the UI: Settings ▸ Modules ▸ Update Apps List ▸
Install.) Then open `/recon`; the anonymous public board is at
`/p/recon` and 404s for tenants without the module installed.

## What you got

- **List view** with search/sort/filter/pagination (`handlers.rs::list_items`)
- **Create form** with sequence-generated codes (`REC/000001`)
