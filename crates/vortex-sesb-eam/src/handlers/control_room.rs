...
async fn control_room(
    ...
    let content = match render_control_room(&db, region, tab).await {
        Ok(html) => html,
...
async fn render_control_room(db: &PgPool, region: Option<Uuid>, tab: &str) -> Result<String, vortex_plugin_sdk::sqlx::Error> {
    // ─── KPIs ───────────────────────────────────────────────────────────────
    let k = Kpis {
        open_mo: scalar_i64(db, &format!("SELECT COUNT(*)::bigint FROM eam_maintenance m WHERE m.state IN {OPEN_MO} {}", rc(region, "m.region_id")), region).await,
        overdue_mo: scalar_i64(db, &format!("SELECT COUNT(*)::bigint FROM eam_maintenance m WHERE m.state IN {OPEN_MO} AND m.scheduled_date < CURRENT_DATE {}", rc(region, "m.region_id")), region).await,
        emergency_mo: scalar_i64(db, &format!("SELECT COUNT(*)::bigint FROM eam_maintenance m WHERE m.state IN {OPEN_MO} AND m.maintenance_type='emergency' {}", rc(region, "m.region_id")), region).await,
        open_defects: scalar_i64(db, &format!("SELECT COUNT(*)::bigint FROM eam_defect d WHERE d.state IN {OPEN_DEF} {}", rc(region, "d.region_id")), region).await,
        critical_defects: scalar_i64(db, &format!("SELECT COUNT(*)::bigint FROM eam_defect d WHERE d.state IN {OPEN_DEF} AND d.severity='critical' {}", rc(region, "d.region_id")), region).await,
        critical_assets: scalar_i64(db, &format!("SELECT COUNT(*)::bigint FROM eam_equipment e WHERE e.condition_status IN ('poor','critical') {}", rc(region, "e.region_id")), region).await,
        substations: scalar_i64(db, &format!("SELECT COUNT(*)::bigint FROM eam_substation s JOIN eam_site si ON si.id=s.site_id WHERE s.active {}", rc(region, "si.region_id")), region).await,
        towers: scalar_i64(db, &format!("SELECT COUNT(*)::bigint FROM eam_transmission_tower t WHERE t.active {}", rc(region, "t.region_id")), region).await,
    };
    ...
    let base = super::analytics::reliability(db, region, &ytd_from, &now.to_rfc3339(), true).await;
    let ci = scalar_i64(db, &format!("SELECT COALESCE(SUM(customers_affected),0)::bigint FROM eam_outage o WHERE o.start_datetime >= '{ytd_from}'::timestamptz AND NOT o.is_major_event {}", rc(region, "o.region_id")), region).await;
    let ongoing = scalar_i64(db, &format!("SELECT COUNT(*)::bigint FROM eam_outage o WHERE o.state='ongoing' {}", rc(region, "o.region_id")), region).await;
    ...
    let assets = gather_assets(db, region).await;

    // ─── feeds ───────────────────────────────────────────────────────────────
    let outages = feed_outages(db, region).await;
    let work_orders = feed_work_orders(db, region).await;
    let defects = feed_defects(db, region).await;
    let condition = feed_condition(db, region).await;
    let overdue = feed_overdue(db, region).await;

    // ─── analytics ───────────────────────────────────────────────────────────
    let analytics = gather_analytics(db, region).await;
...
async fn gather_assets(db: &PgPool, region: Option<Uuid>) -> Vec<Asset> {
    let mut out = Vec::new();
    // Substations
    let sub_sql = format!(
        "SELECT s.id, s.code, s.name, s.state, \
         ...
         FROM eam_substation s JOIN eam_site si ON si.id=s.site_id WHERE s.active {rc} ORDER BY s.code LIMIT 500",
        rc = rc(region, "si.region_id"));
    ...
    let tw_sql = format!(
        "SELECT t.id, t.code, t.name, t.condition_status, t.operational_status, \
         ...
         FROM eam_transmission_tower t WHERE t.active {rc} ORDER BY t.code LIMIT 500",
        rc = rc(region, "t.region_id"));
...
async fn feed_work_orders(db: &PgPool, region: Option<Uuid>) -> Vec<Value> {
    let sql = format!(
        "SELECT m.id, m.name, m.description, m.maintenance_type, m.priority, m.state, e.name AS equip, \
         ...
         WHERE m.state IN {OPEN_MO} {rc} ORDER BY m.priority DESC, m.scheduled_date ASC NULLS LAST, m.id DESC LIMIT 12",
        rc = rc(region, "m.region_id"));
...
async fn feed_overdue(db: &PgPool, region: Option<Uuid>) -> Vec<Value> {
    let sql = format!(
        "SELECT m.id, m.name, m.description, COALESCE(s.name, e.name) AS asset_name, (CURRENT_DATE - m.scheduled_date) AS days \
         ...
         WHERE m.state IN {OPEN_MO} AND m.scheduled_date < CURRENT_DATE {rc} ORDER BY (CURRENT_DATE - m.scheduled_date) DESC LIMIT 10",
        rc = rc(region, "m.region_id"));
    let q = vortex_plugin_sdk::sqlx::query(&sql);
    let q = if let Some(r) = region { q.bind(r) } else { q };
...
async fn feed_defects(db: &PgPool, region: Option<Uuid>) -> Vec<Value> {
    let sql = format!(
        "SELECT d.id, d.name, d.title, d.severity, d.state, e.name AS equip, COALESCE(s.name, e.name) AS asset_name \
         ...
         WHERE d.state IN {OPEN_DEF} {rc} ORDER BY CASE d.severity WHEN 'critical' THEN 4 WHEN 'major' THEN 3 WHEN 'moderate' THEN 2 ELSE 1 END DESC, d.discovered_date DESC NULLS LAST, d.id DESC LIMIT 12",
        rc = rc(region, "d.region_id"));
    let q = vortex_plugin_sdk::sqlx::query(&sql);
    let q = if let Some(r) = region { q.bind(r) } else { q };
...
async fn feed_condition(db: &PgPool, region: Option<Uuid>) -> Vec<Value> {
    let sql = format!(
        "SELECT e.id, e.name, e.equipment_category, e.condition_status, COALESCE(s.name, t.name) AS location, \
         ...
         WHERE e.condition_status IN ('poor','critical') {rc} ORDER BY round({HEALTH_SQL}) ASC LIMIT 12",
        rc = rc(region, "e.region_id"));
    let q = vortex_plugin_sdk::sqlx::query(&sql);
    let q = if let Some(r) = region { q.bind(r) } else { q };
...
async fn feed_outages(db: &PgPool, region: Option<Uuid>) -> Vec<Value> {
    let sql = format!(
        "SELECT o.id, o.name, o.outage_type, o.cause_category, o.state, o.customers_affected, o.is_major_event, s.name AS sub, \
         ...
         WHERE o.state <> 'cancelled' {rc} ORDER BY (o.state='ongoing') DESC, o.start_datetime DESC LIMIT 12",
        rc = rc(region, "o.region_id"));
    let q = vortex_plugin_sdk::sqlx::query(&sql);
    let q = if let Some(r) = region { q.bind(r) } else { q };
...
async fn gather_analytics(db: &PgPool, region: Option<Uuid>) -> Value {
    // mo by state / type over open orders
    let by = |col: &'static str| {
        let sql = format!("SELECT {col} AS k, COUNT(*)::bigint AS n FROM eam_maintenance m WHERE m.state IN {OPEN_MO} {rc} GROUP BY {col} ORDER BY n DESC", col = col, rc = rc(region, "m.region_id"));
        let db = db.clone();
        async move {
            let q = vortex_plugin_sdk::sqlx::query(&sql);
            let q = if let Some(r) = region { q.bind(r) } else { q };
    ...
    let done_month = scalar_i64(db, &format!("SELECT COUNT(*)::bigint FROM eam_maintenance m WHERE m.state IN {DONE_MO} AND m.end_date >= '{month_start}'::date {}", rc(region, "m.region_id")), region).await;
    let done_30d = scalar_i64(db, &format!("SELECT COUNT(*)::bigint FROM eam_maintenance m WHERE m.state IN {DONE_MO} AND m.end_date >= NOW() - INTERVAL '30 days' {}", rc(region, "m.region_id")), region).await;
    let avg_dur: f64 = {
        let sql = format!("SELECT COALESCE(AVG(actual_duration_hours),0)::float8 FROM eam_maintenance m WHERE m.state IN {DONE_MO} AND m.end_date >= NOW() - INTERVAL '30 days' AND m.actual_duration_hours IS NOT NULL {}", rc(region, "m.region_id"));
    ...
    let sev = |s: &'static str| {
        let region = region;
        let sql = format!("SELECT COUNT(*)::bigint FROM eam_defect d WHERE d.state IN {OPEN_DEF} AND d.severity='{s}' {}", rc(region, "d.region_id"));
        let db = db.clone();
    ...
    let lh_sql = format!(
        "SELECT e.id, e.name, e.equipment_category, e.condition_status, COALESCE(s.name, t.name) AS location, round({HEALTH_SQL})::int AS hi \
         FROM eam_equipment e LEFT JOIN eam_substation s ON s.id=e.substation_id LEFT JOIN eam_transmission_tower t ON t.id=e.tower_id \
         WHERE e.active {rc} ORDER BY round({HEALTH_SQL}) ASC LIMIT 8", rc = rc(region, "e.region_id"));
