...
| PDF export | wkhtmltopdf / QWeb-PDF | **Shipped (2026-06)** — `vortex_framework::pdf`, headless Chromium via `chromiumoxide`, **optional** behind the `pdf` cargo feature so the default build pulls neither the crate nor a browser. `?format=pdf` on `/reports/run/{id}` renders any report (tabular or template) to PDF; disabled builds degrade to browser Print→PDF. **Not** wkhtmltopdf (EOL/CVEs). Print pages inline a self-hosted stylesheet (no CDN) so output is offline/deterministic. Build with `cargo build --release -p vortex-cli --features pdf`, ship a Chromium/Chrome binary, set `VORTEX_CHROMIUM` to its path (else auto-detected). See §6. | — |
| User-authored reports | QWeb + report builder | **Shipped (2026-06)** — `vortex_framework::user_reports`, DB-stored so non-developers add reports with no code change. Two shapes: **tabular** (pick model + columns + filters + group-by + count/sum/avg/min/max; safe SQL built from the `ir_model`/`ir_model_field` registry — identifiers allow-listed, values bound, many2one resolved to the related name) and **template** (a sandboxed QWeb-like engine: `{{ field }}`, `{% for r in records %}`, `{% if %}` — data auto-escaped, markup trusted). Output HTML (print→PDF) / CSV / JSON; runs are audited (`BulkExport`). UI: **Settings ▸ Reports** builder gated to admins + the seeded **Report Author** role; **`/reports`** hub + **`/reports/run/{id}`** runner with per-report `required_role`. Tables `ir_report` / `ir_report_column` / `ir_report_filter`, migration 128. | — |
| Outbound email / SMTP | `ir.mail_server` + `mail.mail` | **Shipped (2026-06)** — `vortex_framework::mail` over `lettre` (rustls, no system openssl). Per-tenant `mail_servers` with provider presets (Gmail / Microsoft 365 / SendGrid / Mailgun / generic), passwords encrypted at rest (AES-256-GCM via `vortex_security::crypto`, master key from `VORTEX_SECRET_KEY` — see §6), append-only `mail_log`, and an admin **Settings ▸ Email** UI with a send-test button. Any module sends via `mail::send_default(&db, &EmailMessage, "context")`; the default server and master key make this a one-line call. Migration 127. Reference consumer: approval-request notifications email the next approver. | — |
| Durable background-job queue | `ir.cron` queue + `queue_job` (OCA) | **Shipped (2026-06)** — `vortex_framework::jobs`, a retryable, audited job queue. Central table `ir_job` lives in the **primary** DB (migration 129); each job carries `db_name` so the worker resolves the right tenant pool (`JobContext::pool()`). Worker (`JobWorker::start`, spawned at boot beside the scheduler) claims batches with `FOR UPDATE SKIP LOCKED`, runs each handler in an isolated `tokio::spawn` (panic → failed job, never stuck), retries with exponential backoff (`2^attempts` min, capped 60), and dead-letters after `max_attempts`. Enqueue via `jobs::enqueue(&state.db, NewJob::new(kind, payload).for_db(db).max_attempts(n)…)`; plugins register `kind` handlers on `JobRegistry`. Core handler: `mail.send`. Admin **Settings ▸ Background Jobs** (`/settings/jobs`) lists status + retry/cancel. First consumer: approval notifications now enqueue per-approver `mail.send` jobs instead of fire-and-forget. | — |
| Public REST API + tokens | External API / JSON-RPC + `res.users.apikeys` | **Shipped (2026-06)** — `vortex_framework::api`, a bearer-authenticated `/api/v1` surface for machine-to-machine integration. A token (`api_tokens`, migration 130) authenticates **as a user**, inheriting that user's roles; the raw secret (`vtx_…`) is shown once and only its SHA-256 hash is stored (the `sessions` scheme). Per-tenant: a request names its database via header `X-Vortex-Database` (default DB if absent), mirroring the login cookie's `db\|token`. Generic record CRUD over the model registry: `GET /api/v1/{model}` (paged list + equality filters), `GET/POST/PATCH/DELETE /api/v1/{model}/{id}`, plus `whoami` and `models`. **Two allow-lists keep it safe**: reads project only registered fields (`ir_model_field`, via `jsonb_build_object` — unregistered columns never leak); writes allow-list real table columns (`information_schema`, minus protected `id`/`created_at`/`updated_at`/`created_by`) and cast each bound value to the column's `udt_name`, so a required column like `company_id` is writable while typos/injection are rejected. Reads need a valid token; writes require the token's `write` scope; every call is audited (`.with_database`). Admin **Settings ▸ API Tokens** (`/settings/api-tokens`) mints (show-once + curl example) / lists / revokes. Deferred: per-model Cedar policies (Cedar is default-deny, so generic gating would block everything — the platform's own generic CRUD isn't Cedar-gated either), OpenAPI spec, cursor pagination, relational expansion. | — |
| Outbound webhooks / event bus | `ir.webhook` / automated-action webhooks | **Shipped (2026-06)** — `vortex_framework::webhooks`, the integration counterpart to the REST API: external systems **subscribe** to events instead of polling. An endpoint (`webhook_endpoints`, migration 131) registers a URL, the event types it wants (empty = all), and an optional signing secret (encrypted at rest, the `mail_servers` scheme). `webhooks::emit(...)` fans an event out to matching endpoints by enqueuing one `webhook.deliver` job each — so **delivery rides the durable job queue** and inherits retries, exponential backoff, and dead-lettering; a flaky receiver never blocks the request that produced the event. The handler signs the JSON body with HMAC-SHA256 (`X-Vortex-Signature: sha256=<hex>`, via a `vortex_security::crypto` helper) and POSTs it with `reqwest` (rustls, bundled roots — no system openssl/certs); every attempt is logged to `webhook_deliveries`. Core emitters: the API record handlers fire `record.created` / `record.updated` / `record.deleted`; plugins emit their own types via the SDK. Admin **Settings ▸ Webhooks** (`/settings/webhooks`) adds/edits/pauses endpoints, sends a test `ping`, and shows recent deliveries. Deferred: per-event payload schemas, delivery-rate caps, signature timestamp-tolerance window, replay. | — |
...
# (optional) PDF report export — adds the headless-Chromium backend:
#   cargo build --release -p vortex-cli --features pdf
# then ship a Chromium/Chrome binary and point VORTEX_CHROMIUM at it (below).
...
# Secrets (env-only) — set these in the service environment (systemd unit,
# container env, or an /etc/vortex/vortex.env sourced by the unit). They must
# persist across restarts and stay stable for the life of the deployment.

# (a) Audit chain signing key (Ed25519). Dev / simple deployments only —
#     production should use the PKCS#11 / HSM backend instead (see §7 item 6).
export VORTEX_AUDIT_SIGNING_KEY=$(openssl genpkey -algorithm Ed25519 \
  | openssl pkcs8 -topk8 -nocrypt -outform DER | base64 -w0)

# (b) Secret-encryption master key (AES-256-GCM). Encrypts secrets stored at
#     rest in the tenant databases — today: SMTP passwords in
#     mail_servers.password_enc (Settings ▸ Email). base64 of 32 random bytes.
#     CRITICAL: this key MUST be stable. Rotating or losing it makes every
#     previously stored secret undecryptable (re-enter them after a key change).
#     If unset, Vortex falls back to a fixed INSECURE development key and logs a
#     warning at startup — never run production without setting a real key.
export VORTEX_SECRET_KEY=$(openssl rand -base64 32)

# (c) Chromium path for PDF report export — ONLY needed if the binary was built
#     with `--features pdf`. Point at a Chromium/Chrome executable; if unset,
#     auto-detection scans PATH and well-known locations. Ship the browser with
#     the deployment for air-gapped installs.
# export VORTEX_CHROMIUM=/usr/lib/chromium/chromium
...
- **Secret-encryption master key** (`VORTEX_SECRET_KEY`) is env-var-sourced; a future enhancement is KMS-backed unseal so the AES key for at-rest secrets (SMTP passwords today) is never a plain env var. Until then, treat it like the signing key: provision once, store in the secret manager, never rotate without re-entering stored secrets.
...
5. ~~**No report engine.**~~ **Partially done (Phase 0.7).** `vortex_framework::reports` ships a registry + `GET /reports/:code` endpoint + `ReportDef` plugin contribution with HTML, CSV, and JSON formats. Every render is audited via `AuditAction::BulkExport`. Plugins declare reports in one block via `Plugin::reports()` and get HTTP delivery for free; direct consumers (scheduled email, export scripts) call `render_report(state, code, params)` to get the same bytes without HTTP. **PDF: done (2026-06)** — `vortex_framework::pdf` adds server-side HTML→PDF via headless Chromium (`chromiumoxide`), gated behind the optional `pdf` cargo feature so the default build carries neither the crate nor a browser. `?format=pdf` on the user-report runner renders any report (tabular or template) to a real PDF; disabled builds fall back to browser Print→PDF. The interface is backend-agnostic (a WeasyPrint/Gotenberg backend can slot behind the same `html_to_pdf` signature). Report/print pages inline a **self-hosted stylesheet** (`vortex_framework::user_reports::REPORT_CSS`, from `report.css`) — no CDN, so PDF rendering is fully offline and deterministic (no JS-JIT/network race before printToPDF). **Still deferred**: XLSX (`vortex-report-xlsx` extension).
...
11. ~~**No durable background-job queue.**~~ **Done (2026-06).** `vortex_framework::jobs` adds a retryable, audited queue (`ir_job`, migration 129). The table lives in the **primary** DB so there's a single worker and no per-tenant polling; jobs carry `db_name` and the handler resolves the tenant pool on demand (sidesteps lazy-pool-registration and per-DB schema drift). `JobWorker` claims work with `FOR UPDATE SKIP LOCKED`, isolates each handler in its own `tokio::spawn` (a panicking handler fails its job rather than wedging the row in `running`), backs off exponentially, and dead-letters after `max_attempts`. Plugins register `kind` handlers on a `JobRegistry`; the core ships `mail.send`. Admin retry/cancel UI at `/settings/jobs`. Approval notifications were migrated off fire-and-forget `tokio::spawn` onto the queue as the reference consumer. **Deferred**: cron-style recurring jobs expressed as queue entries (today recurrence is the separate scheduler), priority-lane workers, and a per-job rate limiter.
12. ~~**No machine-to-machine API.**~~ **Done (2026-06).** `vortex_framework::api` exposes a bearer-authenticated `/api/v1` REST surface so external systems integrate without writing an in-process plugin — the developer-platform keystone. Tokens (`api_tokens`, migration 130) authenticate as a user (inherit roles), are hashed at rest like sessions, scoped (read vs `write`), and per-tenant (selected by `X-Vortex-Database`). Generic record CRUD is driven by the model registry with two distinct allow-lists — registered fields for read projection, real table columns for writes — both with bound parameters and identifier validation; reads need a valid token, writes need `write` scope, and every call is audited. Admin UI at `/settings/api-tokens` (show-once mint, list, revoke). **Deferred**: per-model Cedar policies, an OpenAPI/JSON-Schema descriptor, cursor pagination, and relational (`?expand=`) embedding.
13. ~~**No outbound event delivery.**~~ **Done (2026-06).** `vortex_framework::webhooks` lets external systems subscribe to events rather than poll the API. Endpoints (`webhook_endpoints`, migration 131) carry a URL, subscribed event types (empty = all), and an encrypted signing secret. `emit` fans an event to matching endpoints by enqueuing `webhook.deliver` jobs, so delivery reuses the durable queue's retry/backoff/dead-letter machinery (gap #11) — the first real second consumer of the queue after mail. Bodies are HMAC-SHA256 signed; `reqwest` (rustls) does the POST; attempts are logged in `webhook_deliveries`. The API record handlers are the first emitters (`record.*`); plugins emit custom types through the SDK. Admin UI at `/settings/webhooks` with a test-ping and delivery log. **Deferred**: payload schemas per event, rate caps, signature timestamp tolerance, and a replay endpoint.
