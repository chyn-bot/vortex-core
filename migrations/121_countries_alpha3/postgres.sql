-- Add ISO 3166-1 alpha-3 column to countries table
ALTER TABLE countries ADD COLUMN IF NOT EXISTS alpha3 VARCHAR(3);
CREATE UNIQUE INDEX IF NOT EXISTS idx_countries_alpha3 ON countries(alpha3) WHERE alpha3 IS NOT NULL;

-- Populate alpha-3 codes for all 245 countries
UPDATE countries SET alpha3 = m.alpha3
FROM (VALUES
    ...
    ('EH','ESH'),('YE','YEM'),('ZM','ZMB'),('ZW','ZWE'),('AX','ALA'),
    ('BQ','BES'),('CW','CUW'),('GG','GGY'),('IM','IMN'),('JE','JEY'),
    ('BL','BLM'),('MF','MAF'),('SX','SXM'),('SS','SSD'),('XK','XKX')
) AS m(alpha2, alpha3)
WHERE countries.code = m.alpha2 AND countries.alpha3 IS NULL;

COMMENT ON COLUMN countries.alpha3 IS 'ISO 3166-1 alpha-3 code (MYS, USA, GBR). Used by banking and trade verticals alongside the alpha-2 code column.';
